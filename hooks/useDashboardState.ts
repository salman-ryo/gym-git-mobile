...
export function useDashboardState() {
  ...
  const [powerCelebrationData, setPowerCelebrationData] = useState<PowerScoreBreakdown | null>(null);
  ...
  const fetchStatsOnly = useCallback(async (targetPlan?: WeeklyPlan, currentLogs?: GymLog[]) => {
  ...
  const fetchRoadmapOnly = useCallback(async (targetPlanId?: string) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to fetch roadmap.';
      setRoadmapError(msg);
      return [];
    } finally {
  ...
  }, [fetchDashboardStateOnly, fetchLogsOnly, fetchInventoryOnly, fetchRoadmapOnly, fetchStatsOnly]);
  ...
  const scheduleSnoozeReminder = useCallback((delayMs: number, dateKey: string) => {
    ...
    snoozeTimerRef.current = setTimeout(async () => {
      ...
      if (today === dateKey) {
        ...
        const fetchedStats = await fetchDashboardStats(userPlanRef.current, fetchedLogs);
  ...
  }, [user, fetchDashboardStateOnly, fetchLogsOnly, fetchInventoryOnly, fetchRoadmapOnly, fetchStatsOnly, scheduleSnoozeReminder]);

  const handleDailyCheckInYes = useCallback(async (hours: number, workoutType: WorkoutType, notes?: string) => {
  ...
  const statsQuery: SectionQueryState<Stats | null> = useMemo(() => ({
    ...
    error: statsError,
    refetch: async () => { await fetchStatsOnly(); },
  ...
  const logsQuery: SectionQueryState<GymLog[]> = useMemo(() => ({
    ...
    error: logsError,
    refetch: async () => { await fetchLogsOnly(); },
  ...
  const roadmapQuery: SectionQueryState<RoadmapMilestone[]> = useMemo(() => ({
    ...
    error: roadmapError,
    refetch: async () => { await fetchRoadmapOnly(); },
  }), [roadmapMilestones, isRoadmapLoading, roadmapError, fetchRoadmapOnly]);

  const inventoryQuery: SectionQueryState<{ inventory: UserInventoryItem[]; activeEffects: ActiveItemEffect[]; }> = useMemo(() => ({
    data: { inventory: inventoryItems, activeEffects },
    isLoading: isInventoryLoading,
    error: inventoryError,
    refetch: async () => { await fetchInventoryOnly(); },
  ...
  return {
    ...
    needsPlanSelection: !!(user && !isDashboardStateLoading && !dashboardState?.plan),
  };
}
